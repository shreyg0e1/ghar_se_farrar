      ...
      if (hoveredIndex === index) {
        const scaleValue =
          window.innerWidth < 768
            ...
            ? "1.04"
            : "1.08";
        const rotateValue = Math.random() > 0.5 ? "1deg" : "-1deg";
        return `scale(${scaleValue}) rotate(${rotateValue})`;
      }
      return "scale(1) rotate(0deg)";
    };

    // Calculate box shadow based on hover state
    const getBoxShadow = () => {
      if (hoveredIndex === index) {
        return "0 25px 60px -12px rgba(0, 0, 0, 0.4), 0 0 0 1px rgba(255, 255, 255, 0.1)";
      }
      return "0 4px 20px -2px rgba(0, 0, 0, 0.1)";
    };

    // Calculate z-index based on hover state
    const getZIndex = () => {
      if (hoveredIndex === index) return 20;
      if (hoveredIndex !== null && hoveredIndex !== index) return 5;
      return 10;
    };
